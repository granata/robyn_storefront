...
function productsearchinput() {
    ...
    function searchTemplate() {
        return [
            '<style>',
			'.productSearchInput {max-width: 300px;padding-top: 10px;}',
			'.prod-search-input {border-bottom-right-radius: 4px!important;border-top-right-radius: 4px!important;}',
			'.btn-prod-search {font-size: 20px;color: #92AC3C;background-color: #45413B;}',
			'</style>',
			'<form name="productSearchInput" class="productSearchInput" ng-submit="executeSearch()">',
            '<div class="view-form-icon">',
            '<div class="input-group" style="margin:0 0 0px;">',
            '<input type="text" class="form-control prod-search-input" placeholder="{{\'Search\' | r}} {{\'Products\' | r}}" ng-model="productSearchTerm"/>',
            '<i class="fa fa-search"></i>',
            '<span class="input-group-btn">',
            '<button type="submit" class="btn btn-prod-search" ng-disabled="productSearchTerm == null || productSearchTerm == \'\'"><span><i class="fa fa-search text-primary"></i></span></button>',
            '</span>',
            '</div>',
            '</div>',
            '</form>'
