...
function ngFader($interval) {
        ...
        template: '<div class="ng-fader">'+
			...
			'<ul>' + 
				'<li ng-repeat="image in images" ng-click="toggleStartStop()" ng-swipe-right="sliderBack()" ng-swipe-left="sliderForward()">' +
					'<a href="{{image.link}}" target="{{image.target || \'_self\'}}">' +
						'<img data-ng-src="{{image.src}}" data-ng-alt="{{image.alt}}" ng-class="show($index)"/>' +
					'</a>' +
				'</li>' +
			'</ul>' + 
    ...
    function link(scope) {
		...
		scope.images = [{
			src: '//www.four51.com/Themes/Custom/0522be56-6d86-4457-a09b-6b8f04e094c1/SSG/ssg-welcome.jpg',
			link: '',
			alt: 'Welcome to the Social Solutions Company Store'
		}];
