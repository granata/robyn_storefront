...
four51.app.filter('excludeUSPS', function() {
    return function(shipper) {

        var results = [];
        var ignore = ["usps first-class mail","usps usps retail ground"];

        angular.forEach(shipper, function(s) {
            var match = false;
            angular.forEach(ignore,function(prefix){
               if(s.Name.toLowerCase().indexOf(prefix) == 0) match = true;
            });
...
four51.app.filter('USPSStripDays', function() {
    return function(name) {

        return name.replace(/^(usps.*?)\s+\d+\s*-?\s*day.*/i,"$1");
