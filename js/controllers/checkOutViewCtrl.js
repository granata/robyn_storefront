...
function ($scope, $routeParams, $location, $filter, $rootScope, $451, User, Order, OrderConfig, FavoriteOrder, AddressList, GoogleAnalytics) {
	...
	$scope.allDownload = false;
	$scope.downloadCount = 0;
	try{
	angular.forEach($scope.currentOrder.LineItems,function(item){
		if(item.Product.StaticSpecGroups){
			if(item.Product.StaticSpecGroups.Qualities){
				if(item.Product.StaticSpecGroups.Qualities.Specs['Download Only']){
					if(item.Product.StaticSpecGroups.Qualities.Specs['Download Only'].Name == "Download Only"){
						$scope.downloadCount++;
					}
				}
			}
	...
	if($scope.currentOrder.LineItems.length == $scope.downloadCount){
		$scope.allDownload = true;
	}
	} catch(e){}
