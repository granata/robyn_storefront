four51.app.directive('ordershipping', ['Order', 'Shipper', 'Address', 'AddressList', function(Order, Shipper, Address, AddressList) {
	var obj = {
		...
		controller: ['$scope', function($scope) {
			$scope.nonShipAddress = {};
			$scope.nonShipShipper = {};
			...
			AddressList.shipping(function(list) {
				...
				if(list){
					angular.forEach(list,function(listaddress){
						if(listaddress.AddressName == "Non-Shipable"){
							$scope.nonShipAddress = listaddress;
						}
						else{
							$scope.shipaddresses.push(listaddress);
						}
					});
					if($scope.allDownload){
						$scope.shipaddresses = [];
						$scope.shipaddresses.push($scope.nonShipAddress);
						$scope.currentOrder.ShipAddressID = $scope.nonShipAddress.ID;
				...
				saveChanges(
					function(order) {
						Shipper.query(order, function(list) {
							...
							if($scope.allDownload){
								$scope.shippers = [];
								$scope.shippers.push($scope.nonShipShipper);
								$scope.currentOrder.LineItems[0].ShipperName = $scope.nonShipShipper.Name;
								$scope.updateShipper();
							}
							$scope.shippingFetchIndicator = false;
							}
