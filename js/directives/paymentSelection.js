four51.app.directive('paymentselector', function() {
   var obj = {
       ...
       controller: ['$scope', '$rootScope', 'SavedCreditCard', 'SpendingAccount', 'Order', function($scope, $rootScope, SavedCreditCard, SpendingAccount, Order) {
	     ...
	     (function(){
					$scope.restrictedPaymentMethodsText = false;
					$scope.restrictedPaymentMethods = false;
					var
						paymentMethods = {}
						,paymentMethodNames = {}
						,restrictedMethods = {}
						...
						,paymentMethodsDefault = [["PO","PurchaseOrder"],["CC","CreditCard"],["SA","BudgetAccount"],["SFA","Undetermined"]]
						,paymentMethodDefault = -1;

					angular.forEach($scope.currentOrder.LineItems,function(el){
						if("StaticSpecGroups" in el.Product && el.Product.StaticSpecGroups && "Payment" in el.Product.StaticSpecGroups){
							++restrictedItemCount;
							angular.forEach(el.Product.StaticSpecGroups.Payment.Specs,function(el,i){
								i = i.toUpperCase();
								if(!(i in paymentMethods)){
									paymentMethods[i.toUpperCase()] = 1;
									paymentMethodNames[i] = el.Value;
								}
								else ++paymentMethods[i];
							});
						}
					});

					if(restrictedItemCount > 0){
						angular.forEach(paymentMethods,function(el,i){
							if(el == restrictedItemCount){
								restrictedNames.push(paymentMethodNames[i]);
								restrictedMethods[i.toUpperCase()] = el;
								++paymentMethodCount;
							}
						});

						if(paymentMethodCount){
							angular.forEach(paymentMethodsDefault,function(el,i){
								if(el[0] in restrictedMethods && paymentMethodDefault < 0) paymentMethodDefault = i;
							});

							$scope.restrictedPaymentMethods = restrictedMethods;
							$scope.restrictedPaymentMethodsText = restrictedNames.join(", ");
							if(paymentMethodDefault <= paymentMethodCount){
								$scope.setPaymentMethod(paymentMethodsDefault[paymentMethodDefault][1]);
							}
						}
						else{
							$scope.restrictedPaymentMethods = {};
							$scope.restrictedPaymentMethodsText = "empty";
						}
